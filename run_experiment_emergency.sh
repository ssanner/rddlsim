...
cp -f ./dist/EmergencyDomainHOP.jar /tmp/$job_title\_tmp/EmergencyDomainHOP.jar 
...
time /usr/local/common64/jdk1.8/bin/java -jar -Xmx4g -ea EmergencyDomainHOP.jar $job_title\_dom.rddl $job_title\_inst.rddl $lookahead $timeOutMins $viz $numFutures $futureStrat $rootPolicy $job_title\_data.csv $numFolds $trainingFold $testingFold $outFile $nround $randomizeTest  >! $job_title.log  
