			...
			do 
				...
				do 
					...
					do
						...
						JOB_NAME=eval_fitted_emergency_$vehicles\_vehicles_$timeOut\_$lookahead\_$numFutures\_$rootPolicy\_$futureStrat\_round_$round\_long  
						cp run_experiment_eval_fitted_emergency.sh /tmp/emergency/$JOB_NAME.sh
						sed -i "s/DOMAIN_FILENAME/\/nfs\/stak\/students\/n\/nadamuna\/rddlsim\/files\/emergency_domain\/fitting\/fitted_emergency_continuous_time_multiple_vehicles_test.rddl/" /tmp/emergency/$JOB_NAME.sh
						sed -i "s/INSTANCE_FILENAME/\/nfs\/stak\/students\/n\/nadamuna\/rddlsim\/files\/emergency_domain\/fitting\/fitted_inst_emergency_continuous_time_multiple_vehicles_test_$vehicles.rddl/" /tmp/emergency/$JOB_NAME.sh
